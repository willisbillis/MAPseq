...
gex_datafile = "/home/<user>/analysis/tcr/adult/CD8_mtx/"
gex_datatype = '10x_mtx' # other possibilities right now: ['10x_mtx', 'h5ad'] (h5ad from scanpy)
tcr_datafile = '/home/<user>/analysis/tcr/adult/conga_adult_CD8_VDJpaths.csv'
metadata_file = "/home/<user>/analysis/tcr/adult/adult_metadata_conga_all.csv"
organism = 'human'
clones_file = 'adult_tcell.cd8_clones.tsv'
outfile_prefix = 'adult_tcell.cd8'

# this creates the TCRdist 'clones file'
make_10x_clones_file_batch( tcr_datafile, organism, clones_file )
...
plt.savefig("UMAP_"+outfile_prefix+"_GEX.TCR_clusters.pdf", dpi=500)
plt.cla()
...
match_results = conga.tcr_clumping.match_adata_tcrs_to_db_tcrs(
    ...
    num_random_samples_for_bg_freqs=50000, #default is 50000
)
...
# combine alpha and beta hits into a condensed table for later investigation
metadata_df = pd.read_csv(metadata_file)
metadata_df["batch_id"] = metadata_df["batch_id"].fillna(9999).astype(int).astype(str)
metadata_df.fillna("NA", inplace=True)

alpha_hits["batch_id"] = [umi_list.split(",") for umi_list in alpha_hits["cdr3a_match_UMI"]]
for idx, umi_list in enumerate(alpha_hits["batch_id"]):
    alpha_hits["batch_id"].iloc[idx] = ",".join([re.sub("-","",umi[-2:]) for umi in umi_list])

beta_hits["batch_id"] = [umi_list.split(",") for umi_list in beta_hits["cdr3b_match_UMI"]]
for idx, umi_list in enumerate(beta_hits["batch_id"]):
    beta_hits["batch_id"].iloc[idx] = ",".join([re.sub("-","",umi[-2:]) for umi in umi_list])

alpha_hits.rename(columns = {"cdr3a_match_UMI":"match_UMI",
                   "cdr3a_match_gex_clusters":"match_gex_clusters",
                   "cdr3a_match_tcr_clusters":"match_tcr_clusters"}, inplace=True)
beta_hits.rename(columns = {"cdr3b_match_UMI":"match_UMI",
                  "cdr3b_match_gex_clusters":"match_gex_clusters",
                  "cdr3b_match_tcr_clusters":"match_tcr_clusters"}, inplace=True)
alpha_hits["chain"] = ["alpha"] * alpha_hits.shape[0]
beta_hits["chain"] = ["beta"] * beta_hits.shape[0]

all_cdr_hits = pd.concat((alpha_hits, beta_hits), axis=0)

metadata_keepcols = ["donor","infection","time_point","library_id"]
orig_keepcols = ["epitope","epitope_gene","epitope_species","mhc","match_gex_clusters","match_tcr_clusters","chain"]

for md_col in metadata_keepcols:
    all_cdr_hits[md_col] = ["NA"] * all_cdr_hits.shape[0]
    for idx, batch_id_list in enumerate(all_cdr_hits["batch_id"]):
        all_cdr_hits[md_col].iloc[idx] = ",".join([metadata_df[md_col].loc[metadata_df["batch_id"] == batch_id].values[0] for batch_id in batch_id_list.split(",")])

all_cdr_hits = all_cdr_hits[orig_keepcols+metadata_keepcols]

all_cdr_hits.to_excel(f"cdrhits.summary_{outfile_prefix}.xlsx", index=False)
...
plt.savefig("UMAP_"+outfile_prefix+"_GEX.TCR_congascore.pdf", dpi=500)
...
gex_header_tcr_score_names = ['imhc', 'cdr3len', 'mait', 'nndists_tcr']
...
pngfile = outfile_prefix+'_tcr_nbr_graph_vs_gex_features.pdf'
print('making:', pngfile)
conga.plotting.plot_ranked_strings_on_cells(
    adata, results_df, 'X_tcr_2d', 'clone_index', 'mwu_pvalue_adj', 1.0, 'feature', pngfile)

pngfile = outfile_prefix+'_tcr_nbr_graph_vs_gex_features_panels.pdf'
print('making:', pngfile)
...
for nbr_frac in nbr_fracs:
    nbrs_gex, nbrs_tcr = all_nbrs[nbr_frac]
    results.append( conga.correlations.gex_nbrhood_rank_tcr_scores(adata, nbrs_gex, tcr_score_names, pval_threshold, verbose=False ))
    results[-1]['nbr_frac'] = nbr_frac
results_df = pd.concat(results, ignore_index=True)
...
pngfile = outfile_prefix+'_gex_nbr_graph_vs_tcr_features.pdf'
print('making:', pngfile)
...
pngfile = outfile_prefix+'_gex_nbr_graph_vs_tcr_features_panels.pdf'
print('making:', pngfile)
