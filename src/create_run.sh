...
function create_ms_aggr_run() {
    ...
    printf '%s\n' library_id hashtag patient_id | paste -sd ',' >> $1/pipeline/RNA.FB.VDJ/hashtag_demux_ref.csv
    ln -s $MAPSEQ_REPO_PATH/src/aggr_template/pipeline/RNA.FB.VDJ/* $1/pipeline/RNA.FB.VDJ
    printf '%s\n' library_id hashtag patient_id | paste -sd ',' >> $1/pipeline/ATAC.ASAP/hashtag_demux_ref.csv
