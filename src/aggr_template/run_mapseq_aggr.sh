...
cd $PROJECT_PATH/pipeline/RNA.FB.VDJ && $PROJECT_PATH/pipeline/RNA.FB.VDJ/run_cellranger.aggr_rna.sh &
# run cellranger aggr on the ATAC samples
cd $PROJECT_PATH/pipeline/ATAC.ASAP && $PROJECT_PATH/pipeline/ATAC.ASAP/run_cellranger.aggr_atac.sh &
wait

# load RNA cellranger matrices into Seurat and generate a demultiplexed raw Seurat object
cd $PROJECT_PATH/analysis/RNA.FB.VDJ && Rscript $PROJECT_PATH/analysis/run_seurat.demux_rna.R &
# load ATAC cellranger/kite matrices into Seurat and generate a demultiplexed raw Seurat object
cd $PROJECT_PATH/analysis/ATAC.ASAP && Rscript $PROJECT_PATH/analysis/run_seurat.demux_atac.R &
